...
export default {
  ...
  data() {
    return {
      ...
      chainOptions: this.$auth.user.polkadotEvmAccount ? config.polkadotEvmChains : this.$auth.user.ethAccount ? config.evmChains : this.$auth.user.tezosAccount? config.tezosChains : this.$auth.user.polkadotAccount ? config.polkadotChains:  config.nearChains ,
      adjustModal: false,
    }
  },
  components: {
  ...
  computed: {
    filteredList() {
      ...
      } else if (this.nfts.polkadotEvmNft) {
        console.log(this.nfts.polkadotEvmNft)
        return this.nfts.polkadotEvmNft.filter(nft => {return nft.nftMetadata}).map(nft => {
          return {
            id: { tokenId: nft.tokenId },
            metadata: {
              image: nft.nftMetadata.image,
  ...
  async asyncData ({ $axios, $auth, route, store }) {
    if($auth.user.ethAccount != null || $auth.user.tezosAccount != null || $auth.user.polkadotAccount != null || $auth.user.polkadotEvmAccount || $auth.user.nearAccount != null) {
      const account= $auth.user.ethAccount ? $auth.user.ethAccount: $auth.user.tezosAccount ? $auth.user.tezosAccount : $auth.user.polkadotAccount ? $auth.user.polkadotAccount : $auth.user.polkadotEvmAccount? $auth.user.polkadotEvmAccount: $auth.user.nearAccount
